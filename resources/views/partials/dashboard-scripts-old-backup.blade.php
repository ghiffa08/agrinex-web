    ...
    function dashboard() {
        return {
            ...
            async loadChartData() {
                ...
                try {
                    const response = await fetch('/api/v1/dashboard/charts?type=all&minutes=30');
                    const jsonData = await response.json();
            ...
            loadEnvStats() {
                // Load weather data from dashboard API
                fetch('/api/v1/dashboard/weather')
                    ...
                    .then(data => {
                        ...
                        if (weatherData) {
                            ...
                            if (weatherData.temp != null) this.updateMetric('temp', parseFloat(weatherData.temp),
                                'now');
                            if (weatherData.humidity != null) this.updateMetric('humidity', parseFloat(weatherData
                                .humidity), 'BMKG');
                            if (weatherData.light_pct != null) this.updateMetric('light', Math.round(weatherData
                                .light_pct), 'BMKG');
                            if (weatherData.wind_speed != null) this.updateMetric('wind', parseFloat(weatherData
                                .wind_speed), weatherData.wind_dir || '');
                        ...
                        // This is separate from current weather and needed for weekly-tasks component
                        this.loadBMKGDirect();
                    })
                    .catch(error => {
                        console.warn('Weather API not available, trying BMKG direct', error);
                        // Fallback to original BMKG implementation
                        this.loadBMKGDirect();
                    });
            },
            loadBMKGDirect() {
                            ...
                            .then(raw => {
                                ...
                                try {
                                    ...
                                    if (Array.isArray(blocks)) {
                                        const flat = [];
                                        blocks.forEach(block => Array.isArray(block) && block.forEach(entry =>
                                            flat.push(entry)));
