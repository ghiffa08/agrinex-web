...
<div class="flex flex-wrap items-center justify-between gap-3 mb-6">
    <div class="flex items-center gap-3">
        ...
        <div>
            <h2 class="text-xl font-semibold text-gray-800 dark:text-white/90 font-bold">Irrigate Log Details #{{ $log->id }}</h2>
            <nav>
    ...
    <div class="flex items-center gap-3">
        ...
        <a href="{{ route('admin.irrigate-logs.edit', $log->id) }}"
        ...
        <div class="p-6">
            <div class="space-y-4">
                ...
                <div class="flex justify-between items-center pt-4 border-t border-gray-100 dark:border-gray-800">
                    ...
                    <span class="text-theme-sm font-medium text-gray-800 dark:text-white/90">{{ $log->waktu_akhir ? $log->waktu_akhir->format('d-m-Y H:i:s') : '-' }}</span>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="p-6">
            <div class="space-y-4">
                <div class="flex justify-between items-center">
                    ...
                    <span class="rounded-full bg-success-50 px-2.5 py-0.5 text-theme-xs font-semibold text-success-600 dark:bg-success-500/15">{{ $log->node_sukses ?? 0 }}</span>
                </div>
                <div class="flex justify-between items-center pt-4 border-t border-gray-100 dark:border-gray-800">
                    <span class="text-theme-sm text-gray-500 dark:text-gray-400 font-medium">Node Gagal</span>
                    <span class="rounded-full bg-error-50 px-2.5 py-0.5 text-theme-xs font-semibold text-error-600 dark:bg-error-500/15">{{ $log->node_gagal ?? 0 }}</span>
                </div>
                <div class="flex justify-between items-center pt-4 border-t border-gray-100 dark:border-gray-800">
                    <span class="text-theme-sm text-gray-500 dark:text-gray-400 font-medium">Valve ON Akhir</span>
...
<div class="rounded-2xl border border-error-200 bg-error-50/50 p-6 dark:border-error-500/20 dark:bg-error-500/5">
    ...
    <p class="text-theme-sm text-error-700 dark:text-error-300/80 mb-5">Once you delete this irrigation log, there is no going back. Please be certain.</p>
    <form action="{{ route('admin.irrigate-logs.destroy', $log->id) }}" method="POST" 
          onsubmit="return confirm('Hapus log irigasi ini?');">
        @csrf
        @method('DELETE')
