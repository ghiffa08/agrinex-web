        ...
        <div class="col-md-8">
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item"><a href="{{ route('admin.getdata-logs.show', $log->id) }}">Detail #{{ $log->id }}</a></li>
                    <li class="breadcrumb-item active" aria-current="page">Edit</li>
                </ol>
            </nav>
        </div>
        <div class="col-md-4 text-end">
            <a href="{{ route('admin.getdata-logs.show', $log->id) }}" class="btn btn-secondary">
        ...
        <div class="col-md-8">
            <div class="card">
                <div class="card-header bg-primary text-white">
                    <i class="bi bi-pencil"></i> Edit Getdata Log
                </div>
                <div class="card-body">
                    <form action="{{ route('admin.getdata-logs.update', $log->id) }}" method="POST">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control" id="status" name="status" maxlength="255" value="{{ old('status', $log->status) }}" required>
                        </div>
                        <div class="mb-3">
                        ...
                        <a href="{{ route('admin.getdata-logs.show', $log->id) }}" class="btn btn-secondary ms-2">Batal</a>
                    </form>
                </div>
            </div>
        </div>
        <div class="col-md-4">
            <div class="card mb-3">
                ...
                <div class="card-body">
                    ...
                    <p><strong>Sesi ID Getdata:</strong> {{ $log->sesi_id_getdata ?? '-' }}</p>
                    <p><strong>Waktu Mulai:</strong> {{ $log->waktu_mulai ? $log->waktu_mulai->format('d-m-Y H:i:s') : '-' }}</p>
                    <p><strong>Waktu Selesai:</strong> {{ $log->waktu_selesai ? $log->waktu_selesai->format('d-m-Y H:i:s') : '-' }}</p>
                </div>
            </div>
            <div class="card">
                ...
                <div class="card-body">
                    <ul class="mb-0">
                        ...
                        <li>Status dapat diisi dengan keterangan singkat proses.</li>
                        <li>Keterangan dapat dikosongkan jika tidak ada catatan tambahan.</li>
                    </ul>
                </div>
            </div>
        </div>
