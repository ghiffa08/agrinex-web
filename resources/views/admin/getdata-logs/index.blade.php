...
<div class="flex flex-wrap items-center justify-between gap-3 mb-6">
    <h2 class="text-xl font-semibold text-gray-800 dark:text-white/90">Getdata Session Logs</h2>
    <nav>
        <ol class="flex items-center gap-1.5">
            ...
            <li class="text-sm text-gray-800 dark:text-white/90">Getdata Logs</li>
        </ol>
    </nav>
</div>
    ...
    <div x-data="{ showFilter: false }" class="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
        <div class="px-6 py-4 border-b border-gray-100 dark:border-gray-800 flex justify-between items-center">
            ...
            <div class="flex items-center gap-3">
                ...
                <a href="{{ route('admin.getdata-logs.index') }}" class="inline-flex items-center gap-2 rounded-lg border border-gray-300 bg-white px-3 py-1.5 text-theme-sm font-medium text-gray-700 shadow-theme-xs hover:bg-gray-50 hover:text-gray-800 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-white/[0.03] dark:hover:text-gray-200">
                    <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
        ...
        <div x-show="showFilter" x-transition class="border-b border-gray-100 dark:border-gray-800 bg-gray-50/30 dark:bg-gray-900/30 p-6" style="display: none;">
            <form method="GET" action="{{ route('admin.getdata-logs.index') }}" class="grid grid-cols-1 md:grid-cols-4 gap-5">
                <div>
                    <label class="mb-1.5 block text-theme-sm font-medium text-gray-700 dark:text-gray-300">Status</label>
                    <input type="text" name="status" value="{{ request('status') }}" placeholder="Status" class="w-full rounded-lg border border-gray-300 bg-transparent px-3 py-2 text-gray-800 focus:border-brand-500 focus:outline-none focus:ring-1 focus:ring-brand-500 dark:border-gray-700 dark:text-white/90 dark:focus:border-brand-500">
                </div>
                <div>
            ...
            <table class="min-w-full">
                <thead>
                    <tr class="border-b border-gray-100 dark:border-gray-800 bg-gray-50/50 dark:bg-gray-900/50">
                        ...
                        <th class="py-3 px-6 text-left font-medium text-gray-500 text-theme-xs dark:text-gray-400 uppercase tracking-wider">Waktu Selesai</th>
                ...
                <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                    ...
                    <tr class="hover:bg-gray-50 dark:hover:bg-white/[0.02] transition-colors">
                        ...
                        <td class="py-3 px-6 whitespace-nowrap text-theme-sm font-semibold text-gray-800 dark:text-white">{{ $log->sesi_id_getdata }}</td>
                        ...
                        <td class="py-3 px-6 whitespace-nowrap text-center">
                            <span class="rounded-full bg-success-50 px-2 py-0.5 text-theme-xs font-semibold text-success-600 dark:bg-success-500/15 dark:text-success-400">{{ $log->node_sukses }}</span>
                        </td>
                        <td class="py-3 px-6 whitespace-nowrap text-center">
                            <span class="rounded-full bg-error-50 px-2 py-0.5 text-theme-xs font-semibold text-error-600 dark:bg-error-500/15 dark:text-error-400">{{ $log->node_gagal }}</span>
                        </td>
                        <td class="py-3 px-6 whitespace-nowrap text-theme-sm font-semibold text-brand-600 dark:text-brand-400">
                            {{ $log->status }}
                        </td>
                        <td class="py-3 px-6 text-theme-sm text-gray-500 dark:text-gray-400 max-w-xs truncate">
                            {{ $log->keterangan }}
                        </td>
                        <td class="py-3 px-6 whitespace-nowrap">
                            <div class="flex items-center justify-center gap-2">
                                <a href="{{ route('admin.getdata-logs.show', $log->id) }}" class="text-brand-500 hover:text-brand-600 transition-colors" title="View">
                                ...
                                <a href="{{ route('admin.getdata-logs.edit', $log->id) }}" class="text-warning-500 hover:text-warning-600 transition-colors" title="Edit">
                                    <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                ...
                                <form action="{{ route('admin.getdata-logs.destroy', $log->id) }}" method="POST" class="inline-block" onsubmit="return confirm('Are you sure you want to delete this record?');">
                                    @csrf
                                    @method('DELETE')
                    ...
                    @empty
                    <tr>
                        <td colspan="9" class="py-10 px-6 text-center">
