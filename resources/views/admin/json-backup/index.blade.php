...
<div class="flex flex-wrap items-center justify-between gap-3 mb-6">
    <h2 class="text-xl font-semibold text-gray-800 dark:text-white/90">JSON Backup Data</h2>
    <nav>
        <ol class="flex items-center gap-1.5">
            ...
            <li class="text-sm text-gray-800 dark:text-white/90 font-medium">JSON Backup</li>
        </ol>
    </nav>
</div>
    ...
    <div x-data="{ showFilter: false }" class="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
        <div class="px-6 py-4 border-b border-gray-100 dark:border-gray-800 flex justify-between items-center">
            ...
            <div class="flex items-center gap-3">
                ...
                <a href="{{ route('admin.json-backup.index') }}" class="inline-flex items-center gap-2 rounded-lg border border-gray-300 bg-white px-3 py-1.5 text-theme-sm font-medium text-gray-700 shadow-theme-xs hover:bg-gray-50 hover:text-gray-800 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-white/[0.03] dark:hover:text-gray-200">
                    <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
        ...
        <div x-show="showFilter" x-transition class="border-b border-gray-100 dark:border-gray-800 bg-gray-50/30 dark:bg-gray-900/30 p-6" style="display: none;">
            <form method="GET" action="{{ route('admin.json-backup.index') }}" class="grid grid-cols-1 md:grid-cols-4 gap-5">
                <div>
            ...
            <table class="min-w-full">
                <thead>
                    <tr class="border-b border-gray-100 dark:border-gray-800 bg-gray-50/50 dark:bg-gray-900/50">
                        ...
                        <th class="py-3 px-6 text-right font-medium text-gray-500 text-theme-xs dark:text-gray-400 uppercase tracking-wider">Data Size (KB)</th>
                        <th class="py-3 px-6 text-right font-medium text-gray-500 text-theme-xs dark:text-gray-400 uppercase tracking-wider">Total Records</th>
                        <th class="py-3 px-6 text-left font-medium text-gray-500 text-theme-xs dark:text-gray-400 uppercase tracking-wider">Node Completeness</th>
                        <th class="py-3 px-6 text-right font-medium text-gray-500 text-theme-xs dark:text-gray-400 uppercase tracking-wider">Getdata Logs</th>
                        <th class="py-3 px-6 text-right font-medium text-gray-500 text-theme-xs dark:text-gray-400 uppercase tracking-wider">Sensor Weather</th>
                        <th class="py-3 px-6 text-right font-medium text-gray-500 text-theme-xs dark:text-gray-400 uppercase tracking-wider">Sensor Node</th>
                        <th class="py-3 px-6 text-left font-medium text-gray-500 text-theme-xs dark:text-gray-400 uppercase tracking-wider">Backup Timestamp</th>
                ...
                <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                    ...
                    <tr class="hover:bg-gray-50 dark:hover:bg-white/[0.02] transition-colors">
                        <td class="py-3 px-6 whitespace-nowrap">
                            <span class="rounded-full bg-gray-100 px-2 py-0.5 text-theme-xs font-medium text-gray-600 dark:bg-gray-800 dark:text-gray-400">{{ $backup->id }}</span>
                        </td>
                        <td class="py-3 px-6 whitespace-nowrap text-theme-sm font-semibold text-gray-800 dark:text-white">{{ $backup->sesi_id_getdata }}</td>
                        <td class="py-3 px-6 whitespace-nowrap text-right">
                            <span class="rounded-full bg-sky-50 text-sky-500 dark:bg-sky-500/15 dark:text-sky-400 px-2.5 py-0.5 text-theme-xs font-semibold">
                                {{ number_format($backup->data_size_kb, 2) }} KB
                            </span>
                        </td>
                        <td class="py-3 px-6 whitespace-nowrap text-right text-theme-sm text-gray-700 dark:text-gray-300">{{ $backup->total_records }}</td>
                        <td class="py-3 px-6 text-theme-sm text-gray-500 dark:text-gray-400 max-w-xs truncate">{{ $backup->node_completeness }}</td>
                        <td class="py-3 px-6 whitespace-nowrap text-right text-theme-sm text-gray-700 dark:text-gray-300">{{ $backup->getdata_logs_count }}</td>
                        <td class="py-3 px-6 whitespace-nowrap text-right text-theme-sm text-gray-700 dark:text-gray-300">{{ $backup->sensor_weather_count }}</td>
                        <td class="py-3 px-6 whitespace-nowrap text-right text-theme-sm text-gray-700 dark:text-gray-300">{{ $backup->sensor_node_count }}</td>
                        ...
                        <td class="py-3 px-6 whitespace-nowrap text-center">
                            <div class="flex items-center justify-center gap-2">
                                <a href="{{ route('admin.json-backup.show', $backup->id) }}" class="text-brand-500 hover:text-brand-600 transition-colors" title="View">
                                ...
                                <form action="{{ route('admin.json-backup.destroy', $backup->id) }}" method="POST" class="inline-block" onsubmit="return confirm('Are you sure you want to delete this backup?');">
                                    @csrf
                                    @method('DELETE')
                    ...
                    <tr>
                        <td colspan="10" class="py-10 px-6 text-center">
                            ...
                            <p class="text-gray-500 dark:text-gray-400 text-sm">No backups found.</p>
                        </td>
                    </tr>
                    @endforelse
                </tbody>
        ...
        <div class="px-6 py-4 border-t border-gray-100 dark:border-gray-800">
            <div class="flex flex-col md:flex-row justify-between items-center gap-4">
                <div class="text-sm text-gray-500 dark:text-gray-400">
                    Showing <span class="font-medium text-gray-800 dark:text-white/90">{{ $backups->firstItem() ?? 0 }}</span> to <span class="font-medium text-gray-800 dark:text-white/90">{{ $backups->lastItem() ?? 0 }}</span> of <span class="font-medium text-gray-800 dark:text-white/90">{{ $backups->total() }}</span> entries
                </div>
                <div>
                    {{ $backups->links() }}
                </div>
