...
<div class="flex flex-wrap items-center justify-between gap-3 mb-6">
    <h2 class="text-xl font-semibold text-gray-800 dark:text-white/90 font-bold">Weather Station Data (Node 65)</h2>
    <nav>
        <ol class="flex items-center gap-1.5">
            ...
            <li class="text-sm text-gray-800 dark:text-white/90 font-medium">Weather Data</li>
        </ol>
    </nav>
</div>
    ...
    <div x-data="{ showFilter: false }" class="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
        <div class="px-6 py-4 border-b border-gray-100 dark:border-gray-800 flex justify-between items-center">
            ...
            <div class="flex items-center gap-3">
                ...
                <a href="{{ route('admin.weather-data.index') }}" class="inline-flex items-center gap-2 rounded-lg border border-gray-300 bg-white px-3 py-1.5 text-theme-sm font-medium text-gray-700 shadow-theme-xs hover:bg-gray-50 hover:text-gray-800 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-white/[0.03] dark:hover:text-gray-200">
                    <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
        ...
        <div x-show="showFilter" x-transition class="border-b border-gray-100 dark:border-gray-800 bg-gray-50/30 dark:bg-gray-900/30 p-6" style="display: none;">
            <form method="GET" action="{{ route('admin.weather-data.index') }}" class="grid grid-cols-1 md:grid-cols-4 gap-5">
                <div>
            ...
            <table class="min-w-full">
                <thead>
                    <tr class="border-b border-gray-100 dark:border-gray-800 bg-gray-50/50 dark:bg-gray-900/50">
                        ...
                        <th class="py-3 px-6 text-left font-medium text-gray-500 text-theme-xs dark:text-gray-400 uppercase tracking-wider">Humidity (%)</th>
                        <th class="py-3 px-6 text-right font-medium text-gray-500 text-theme-xs dark:text-gray-400 uppercase tracking-wider">Light (lux)</th>
                        <th class="py-3 px-6 text-right font-medium text-gray-500 text-theme-xs dark:text-gray-400 uppercase tracking-wider">Wind (m/s)</th>
                        <th class="py-3 px-6 text-left font-medium text-gray-500 text-theme-xs dark:text-gray-400 uppercase tracking-wider">Rain</th>
                        ...
                        <th class="py-3 px-6 text-right font-medium text-gray-500 text-theme-xs dark:text-gray-400 uppercase tracking-wider">Power</th>
                ...
                <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                    ...
                    <tr class="hover:bg-gray-50 dark:hover:bg-white/[0.02] transition-colors">
                        ...
                        <td class="py-3 px-6 whitespace-nowrap text-right">
                            <span class="rounded-full bg-brand-50 px-2 py-0.5 text-theme-xs font-semibold text-brand-600 dark:bg-brand-500/15 dark:text-brand-400">
                        ...
                        <td class="py-3 px-6 whitespace-nowrap w-40">
                            <div class="flex items-center gap-2">
                                ...
                                <span class="text-theme-xs text-gray-500 w-8 text-right font-medium">{{ number_format($data->humidity, 0) }}%</span>
                            </div>
                        </td>
                        <td class="py-3 px-6 whitespace-nowrap text-right text-theme-sm text-gray-700 dark:text-gray-300">{{ number_format($data->light, 1) }}</td>
                        <td class="py-3 px-6 whitespace-nowrap text-right text-theme-sm text-gray-700 dark:text-gray-300">{{ number_format($data->wind, 1) }}</td>
                        <td class="py-3 px-6 whitespace-nowrap">
                            @if($data->rain == 0)
                                <span class="rounded-full bg-success-50 px-2 py-0.5 text-theme-xs font-medium text-success-600 dark:bg-success-500/15 dark:text-success-400">No Rain</span>
                            @else
                                <span class="rounded-full bg-error-50 px-2 py-0.5 text-theme-xs font-medium text-error-600 dark:bg-error-500/15 dark:text-error-400">Raining</span>
                            @endif
                        </td>
                        <td class="py-3 px-6 whitespace-nowrap text-right text-theme-sm text-gray-700 dark:text-gray-300">{{ number_format($data->voltage, 2) }}V</td>
                        <td class="py-3 px-6 whitespace-nowrap text-right text-theme-sm text-gray-700 dark:text-gray-300">{{ number_format($data->current, 1) }}mA</td>
                        <td class="py-3 px-6 whitespace-nowrap text-right text-theme-sm text-gray-700 dark:text-gray-300">{{ number_format($data->power, 0) }}mW</td>
                        ...
                        <td class="py-3 px-6 whitespace-nowrap">
                            <div class="flex items-center justify-center gap-2">
                                <a href="{{ route('admin.weather-data.show', $data->id) }}" class="text-brand-500 hover:text-brand-600 transition-colors" title="View">
                                ...
                                <a href="{{ route('admin.weather-data.edit', $data->id) }}" class="text-warning-500 hover:text-warning-600 transition-colors" title="Edit">
                                    <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                ...
                                <form action="{{ route('admin.weather-data.destroy', $data->id) }}" method="POST" class="inline-block" onsubmit="return confirm('Are you sure you want to delete this record?');">
                                    @csrf
                                    @method('DELETE')
                    ...
                    @empty
                    <tr>
                        <td colspan="12" class="py-10 px-6 text-center">
                            ...
                            <p class="text-gray-500 dark:text-gray-400 text-sm">No weather data found.</p>
                        </td>
                    </tr>
                    @endforelse
                </tbody>
        ...
        <div class="px-6 py-4 border-t border-gray-100 dark:border-gray-800">
            <div class="flex flex-col md:flex-row justify-between items-center gap-4">
                <div class="text-sm text-gray-500 dark:text-gray-400">
                    Showing <span class="font-medium text-gray-800 dark:text-white/90">{{ $weatherData->firstItem() ?? 0 }}</span> to <span class="font-medium text-gray-800 dark:text-white/90">{{ $weatherData->lastItem() ?? 0 }}</span> of <span class="font-medium text-gray-800 dark:text-white/90">{{ $weatherData->total() }}</span> entries
                </div>
                <div>
                    {{ $weatherData->links() }}
                </div>
