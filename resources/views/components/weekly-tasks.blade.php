<div class="mt-6 grid md:grid-cols-2 gap-6" x-show="weekViewDays.length">
    <!-- Current Tasks -->
    <div class="bg-white dark:bg-slate-900 border border-gray-100 dark:border-slate-800 rounded-2xl p-6 shadow-sm flex flex-col gap-4">
        ...
        <div class="flex flex-col space-y-4">
            ...
            <template x-if="loadingWeather">
                <div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
                    <template x-for="i in 4">
                        <div class="bg-gray-50 dark:bg-slate-800/50 border border-gray-100 dark:border-slate-700/50 rounded-lg p-3 text-center animate-pulse">
                            <div class="h-4 bg-gray-200 dark:bg-slate-700 rounded w-1/2 mx-auto mb-2"></div>
                            <div class="h-8 w-8 bg-gray-200 dark:bg-slate-700 rounded-full mx-auto mb-2"></div>
                            <div class="h-6 bg-gray-200 dark:bg-slate-700 rounded w-2/3 mx-auto mb-1"></div>
                            <div class="h-3 bg-gray-200 dark:bg-slate-700 rounded w-full mx-auto"></div>
                        </div>
                    </template>
                </div>
            </template>
            ...
            <div x-show="forecastView==='24h' && !loadingWeather" style="display: none;" class="grid grid-cols-2 sm:grid-cols-4 gap-3" x-cloak>
                <template x-for="f in forecast24h" :key="f.local_datetime">
                    <div class="bg-gray-50 dark:bg-slate-800/50 border dark:border-slate-700/50 rounded-lg p-3 text-center hover:bg-gray-100 dark:hover:bg-slate-700 transition">
                        <div class="text-sm font-semibold text-gray-700 dark:text-gray-200 mb-1" x-text="f.hour"></div>
    ...
    <div class="bg-white dark:bg-slate-900 border border-gray-100 dark:border-slate-800 rounded-2xl p-6 shadow-sm flex flex-col gap-4">
            ...
            <div class="space-y-3">
                <template x-for="i in 2">
                    <div class="flex gap-3 items-stretch animate-pulse">
                        <div class="w-12 h-12 shrink-0 rounded-md bg-gray-200 dark:bg-slate-700"></div>
                        <div class="flex-1 bg-white/60 dark:bg-slate-800/60 border border-white/60 dark:border-white/10 rounded-md px-3 py-2 shadow-sm space-y-2">
                            ...
                            <div class="h-3 bg-gray-200 dark:bg-slate-700 rounded w-1/4 mt-2"></div>
                        </div>
                    </div>
                </template>
            </div>
        ...
        <div x-show="!loadingSchedule" style="display: none;" class="space-y-3">
            <template x-for="t in currentTasks" :key="t.id">
                <div class="flex gap-3 items-stretch">
                    ...
                    <div class="flex-1 bg-white/60 dark:bg-slate-800/60 border border-white/60 dark:border-white/10 rounded-md px-3 py-2 shadow-sm">
        ...
        <div class="flex items-center justify-between">
            ...
            <div class="flex gap-1">
                <button class="px-2 py-1 rounded bg-gray-100 hover:bg-gray-200 dark:bg-slate-700 dark:hover:bg-slate-600 transition text-secondary"
        ...
        <div class="flex justify-between text-[11px] font-semibold text-green-800 dark:text-emerald-500 px-1">
            ...
            <template x-for="d in weekViewDays" :key="d.date">
                <div @click="selectWeekDay(d)"
                    :class="['flex-1 relative rounded-2xl py-3 flex flex-col items-center gap-1 cursor-pointer transition-all duration-200 animate-pop',
                        d.categoryBg, d.active ? 'ring-4 ring-green-500 ring-offset-2 ring-offset-white dark:ring-offset-slate-900 shadow-lg scale-[1.05]' : 'hover:shadow dark:hover:shadow-slate-800']">
