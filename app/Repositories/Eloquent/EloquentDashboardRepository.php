...
class EloquentDashboardRepository implements DashboardRepositoryInterface
{
    ...
    private function buildNodeData(int $deviceId): ?array
    {
        ...
        return [
            'id'                    => $device->id,
            'device_id'             => $device->id,
            'name'                  => $device->name ?? "Node {$device->id}",
            'device_name'           => $device->name ?? "Node {$device->id}",
            'plot_number'           => $device->id,
            'location'              => $device->lokasi ?? $device->location ?? 'Unknown',
            'treatment_description' => $device->keterangan ?? 'Device monitoring',
            'treatment_type'        => $device->group ?? 'A',
            'treatment_code'        => $device->kode_perlakuan ?? "P{$device->id}",
            'group'                 => $device->group ?? 'A',
            'kode_perlakuan'        => $device->kode_perlakuan ?? "P{$device->id}",
